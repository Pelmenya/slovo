...
2. **Структурированный архив анализов воды** — 15 504 бланка 2020-2026 превращены в датасет с географической привязкой и **поиском похожих анализов за ~600 мс с комбинированным фильтром «гео + семантика»** (регион + тип источника + глубина + химическая близость).
...
Дальше — 5 направлений развития (4 готовых к реализации + 1 потенциальное расширение Vision на распознавание проблем воды), выбор приоритета за вами.
...
- **Комбинирование «геолокация + семантика»** уже работает: при поиске похожих можно сузить выдачу по региону / городу / типу источника (скважина/колодец/водопровод) / диапазону глубины. Например: «найти 10 анализов в Подмосковье, скважины 30-100 м, химически похожие на мой»
- Точный radius-поиск через PostGIS `ST_DWithin` (5/10/25/50 км от точки) пока зарезервирован — работает текущее (по названию региона), точный radius в Phase 2.5 при появлении prostor-app виджета
- 128 тестов
...
## Что можно делать дальше (5 направлений)
...
### 4. Карточка «Мой анализ воды» с интерактивным сравнением

**Что даёт:** клиент в кабинете PROSTOR открывает свой последний анализ — видит **визуализацию всех параметров** с цветовой индикацией превышений ПДК, **сравнение с похожими анализами рядом** (медиана соседей в радиусе 10 км), и **мини-карту** с точками этих похожих. Конкретно:

- Bar/radar chart по 14 параметрам с покрытием >50% (жёсткость, железо, марганец, мутность, цветность, pH, TDS, нитраты, сульфиды, сероводород, магний, фториды, окисляемость, запах) — цвет по % превышения ПДК (зелёный / жёлтый / оранжевый / красный)
- Сравнительная таблица «моё значение vs медиана соседей в радиусе 10 км vs ПДК» — клиент видит «у меня 1.5 мг/л железа, у соседей в среднем 1.8 мг/л, ПДК 0.3 мг/л — это типичная проблема района»
- Мини-карта с точками похожих анализов (через `/water-analysis/similar`) — клик по точке открывает соседний анализ для сравнения
- Bridge с каталогом: «по параметрам с превышением — посмотреть оборудование» (после получения правил специалиста, направление 2)

**Зрелость:** это **визуализация поверх готовых endpoint'ов** — `/water-analysis/similar` уже работает с **комбинированным фильтром «гео + семантика»** (region / locality / depth / intakeType + embedding). Например запрос «химически похожие анализы у меня в Подмосковье среди скважин 30-100 м» — отрабатывает один HTTP-вызовом. Backend изменений не требуется. Точный radius (ST_DWithin) — Phase 2.5, не блокер для MVP-карточки. Чистая UI-задача.

**Объём работ:** ~3-4 дня (фронт-карточка + bar/radar chart + сравнительная таблица + мини-карта с маркерами + интеграция с `/water-analysis/similar`).

**Зависимости:** ничего критичного. Bridge с каталогом включится автоматически когда появятся правила от специалиста (направление 2), но базовая карточка работает без них.

### 5. Vision-распознавание проблемы воды по фото (потенциальное расширение)
...
| Запрос | Зачем |
|---|---|
| Решение по приоритету направлений (1 / 2 / 3 / 4 / 5) | Чтобы мы сфокусировались на одном-двух пунктах, а не размазывали ресурс |
...
**За апрель-май у нас готовы к показу: поиск по каталогу 155 товаров Аквафор-Pro (по тексту или фото оборудования), структурированный архив 15 504 анализов воды с 19 параметрами (СанПиН 1.2.3685-21, 14 пригодны для агрегации в карту) и поиском похожих за ~600 мс, mobile-first прототип тепловой карты на 3 viewport (iPhone/iPad/Desktop) с интерактивным radius-поиском. Готовы развернуть карту в клиентское приложение PROSTOR (~1 неделя), построить карточку «Мой анализ» с интерактивным сравнением с похожими (~3-4 дня, без зависимостей), сделать алгоритм подбора оборудования (нужны правила от вашего специалиста), подключить real-time ingest из CRM, расширить Vision на распознавание проблем воды по фото — приоритет за вами.**
