...
>
> **Версия документа:** 1.7 (2026-05-19, добавлена секция «Доказательство концепции» — 7 reference Q&A на текущих 155 карточках с дословными цитатами AI-ответов; синхронизировано с реальным кодом feeder'a `crm-aqua-kinetics-back/src/modules/moy-sklad/modules/catalog-sync/` — функции `fetchGroupContext`, `buildContentForEmbedding`, `parseComponentRefs`).
>
...
## В перспективе: каталог как knowledge base экспертного уровня

Этот раздел — для руководства Аквафор-Pro. Объясняет **стратегический эффект** от правильного заполнения карточек, выходящий за рамки smart-search.

Когда менеджеры по этому руководству заполнят 155 карточек (атрибуты пределов очистки, описания, фото в нескольких ракурсах, System Bundle'ы групп с услугами), каталог Аквафор-Pro превратится не просто в поисковый индекс, а в **полноценную базу знаний по водоочистке** для приложения PROSTOR. Получаемые в перспективе сценарии:

### Q&A экспертного уровня

Клиент пишет в чате PROSTOR: «У меня вода из скважины, железо 2 мг/л, жёсткость 9. Что взять?» — AI читает 155 embedding'ов, находит товары проходящие по пределам (`max_iron ≥ 2`, `max_hardness ≥ 9`), и не просто перечисляет, а **объясняет**: «Для вашей воды подходит DWM-101S (max железо 0.5, max жёсткость 12) — он удаляет железо до 0.5 мг/л через обратноосмотическую мембрану. Перед ним желательно поставить предфильтр Аквафор Гросс с Fe-картриджем — он снизит железо с 2 до 0.5 мг/л чтобы основной фильтр работал в паспортных пределах. Установка обоих — 4 500 ₽, монтаж под мойку».

Этот ответ **читается из карточек** через RAG (retrieval-augmented generation) — AI не выдумывает, а ссылается на конкретные товары и их параметры.

### Сравнение моделей

Клиент: «В чём разница между DWM-101S и Фаворит?» → AI вытаскивает атрибуты обоих карточек, сравнивает по параметрам:

| Параметр | DWM-101S | Фаворит |
|---|---|---|
| Тип | обратный осмос | проточный |
| Макс. жёсткость | 12 мг-экв/л | 7 мг-экв/л |
| Удаляет бактерии | да | нет |
| Цена | 16 900 ₽ | 12 490 ₽ |

И комментирует: «DWM-101S сильнее по жёсткой воде, Фаворит дешевле и проще в обслуживании. Если у вас жёсткость до 7 — Фаворит достаточен, если выше — выбирайте обратный осмос».

### Образовательный mode

Клиент: «А как работает обратный осмос?» → AI читает контексты групп («Контекст группы: ...») из всех карточек RO-систем, составляет объяснение из общего паттерна: «Обратный осмос — это технология глубокой очистки воды через мембрану с микропорами 0.0001 мкм. Удаляет 95% солей жёсткости, тяжёлые металлы, бактерии. Состоит из 3-5 ступеней: механическая предочистка, угольная очистка, мембрана, постфильтр.» — это знание **emerges** из совокупности карточек, не пишется отдельно.

### Подбор комплекта (не отдельный товар, а система)

Клиент: «Дача в Подмосковье, скважина 30 м, по анализу железо 1.5 / марганец 0.4 / жёсткость 8.» → AI понимает что нужно **три уровня очистки**:
1. **Предфильтр на стояк** — для механики и железа (через Fe-картридж)
2. **Магистральный** — для остаточного железа и хлора (если есть)
3. **Под мойку** — для питьевой воды (обратный осмос)

И собирает комплект из трёх товаров с обоснованием. Это **systems thinking** который раньше делал только опытный менеджер по телефону.

### Контекстная поддержка после покупки

Клиент пишет: «У меня капает кран после фильтра» → AI через связанные товары (атрибуты «...элемент для обслуживания» + услуги System Bundle) находит applicable сервисную услугу «Ремонт/Замена крана для чистой воды» в нужной категории + предлагает её или направляет к мастеру.

### ROI заполнения растёт нелинейно

Каждая заполненная карточка добавляет несколько вещей в общую базу знаний:

- **+1 точка** для семантического поиска по конкретному товару
- **+N факты** для Q&A экспертного уровня (когда атрибутов много)
- **+K паттерны** для сравнения и подбора комплектов
- **+M контекстных связок** с услугами и расходниками через System Bundle

Когда заполнено 50 карточек — AI справляется с типовыми запросами. 100 карточек — AI обрабатывает сложные edge cases. 155 карточек с полными атрибутами + Phase 2 backend (Q&A chatflow, conversational state) — **PROSTOR превращается в эксперта по водоочистке**, доступного клиенту 24/7 без перерывов на обед.

Это **прямое уменьшение нагрузки на менеджеров-консультантов** (типовые вопросы AI отвечает сам), плюс **рост конверсии** (клиент получает компетентный ответ сразу, не ждёт ответа на телефон).

**Что для этого нужно:**

- **Менеджеры:** заполнить карточки по этому гайду — описание, атрибуты пределов очистки, фото в ракурсах, ссылки на картриджи. Разовая работа на ~6 часов на менеджера.
- **Администратор МС:** создать новые типы атрибутов (одно-два кратные часы работы), настроить System Bundle'ы групп с услугами и описаниями.
- **Slovo team:** Phase 1.5 (structured filter по `max_*` атрибутам, 1-2 недели) + Phase 2 (Q&A chatflow, conversational state, ~1-2 месяца) — это уже наш скоуп.

Без правильно заполненных карточек никакой Phase 2 backend не превратит PROSTOR в эксперта — данные первичны, AI лишь умеет с ними работать. Поэтому заполнение карточек **сейчас** — это инвестиция, окупающаяся в перспективе нелинейным ростом качества AI-поддержки клиентов.

---

## Доказательство концепции (2026-05-19)

Примеры выше — **не теория**. 19 мая 2026 мы прогнали 7 reference запросов через AI поверх **существующих 155 карточек каталога** (текущее состояние, без дополнительного заполнения по этому гайду). Результаты ниже — **дословные цитаты ответов** AI и реально найденные товары из каталога.

Лабораторный журнал PoC: `docs/experiments/knowledge-base-poc/2026-05-19-catalog-qa-baseline.md`.

### Setup за 1 предложение

AI-консультант (Claude Sonnet 4.6) подключён к `catalog-aquaphor` через semantic retrieval — на каждый вопрос находит топ-4 релевантных карточки и формулирует ответ строго на их основе, со ссылкой на конкретные товары.

### Что AI уже умеет на текущих карточках

#### Q: «Чем DWM-101S отличается от Фаворита?»

> **DWM-101S Морион** — это система обратного осмоса. Она удаляет практически любые примеси (включая токсичные вещества и вирусы), умягчает воду и работает при низком давлении. Занимает в 2 раза меньше места, чем классический обратный осмос с баком, и экономит до 9 тонн воды в год.
>
> **Фаворит** — проточный фильтр в стальном корпусе. Рассчитан на повышенное потребление воды (большая семья, офис, детский сад). Использует сменный модуль с мембраной 0,1 микрона — это даёт защиту от бактерий, но технология очистки менее глубокая, чем обратный осмос.
>
> **Коротко:** если нужна максимально глубокая очистка от любых примесей — DWM-101S Морион. Если важен высокий расход воды и более простое обслуживание — Фаворит.

**AI назвал точные модели**, развёл их по технологии, use case и обслуживанию. Источники в ответе: 4 карточки (Аквафор В150 Фаворит, Модуль сменный В150, DWM-102S Pro, DWM-101S).

#### Q: «Удаляет ли ваш фильтр бактерии?»

> Да! Модуль **Аквафор Pro B** оснащён ультрафильтрационной мембраной, поры которой в 5 раз меньше любого микроорганизма — он механически удаляет бактерии и цисты без химических бактерицидов. Вода после такой очистки безопасна для всей семьи, включая детей и людей с чувствительным пищеварением, и не требует кипячения.

**Прямой yes/no с обоснованием**, конкретный модуль (`Pro B`, цена в metadata 3 600 ₽), указано применение для семей с детьми. Источники: 4 карточки.

#### Q: «У меня скважина с железом 1.5 мг/л. Что предложишь?»

> Для вашей задачи подойдёт **Водоочиститель Аквафор Трио Fe H** — он специально разработан для удаления коллоидного железа, а также умягчает воду и защищает от накипи. При уровне железа 1,5 мг/л это решение должно справиться с задачей.
>
> Если нужна предварительная грубая очистка (например, перед основной системой), можно рассмотреть сменные картриджи **Элемент сменный Fe** (форматы 10" или 20") для предфильтра.
>
> Уточните, пожалуйста: вода используется для всего дома или только для питья?

**AI задаёт уточняющий вопрос** клиенту — это поведение консультанта, не справочника. Source — Трио Fe H + 2 Fe-картриджа.

#### Q: «Как работает обратный осмос?»

> Обратный осмос — это технология ультратонкой очистки воды через специальную мембрану, которая задерживает даже самые мелкие загрязнители: токсичные вещества, бактерии и вирусы. Вода проходит через несколько ступеней фильтрации, а концентрированный поток загрязнителей отводится в дренаж.
>
> Например, в системах Аквафор серии DWM и Osmo Pro реализована рециркуляция воды внутри системы — это минимизирует её расход и делает процесс экологичным и экономичным.

**Образовательный режим** — клиент учится через AI, освобождая менеджера для закрытия сделки. Аквафор-специфичная деталь (рециркуляция в DWM и Osmo Pro) подтянулась из карточек этих серий.

### Что AI признал «не знаю» — и что это значит

Из 7 запросов **2 показали границы текущих карточек** — и это самый ценный сигнал для понимания где заполнение карточек даст немедленный эффект.

#### Q: «Подбери фильтр для квартиры с бюджетом до 15 000 ₽»

> Из каталога могу предложить **Аквафор Кристалл А** — это бюджетная система под мойку. Также есть **Аквафор В150 Фаворит** — надёжный проточный фильтр в стальном корпусе.
>
> ⚠️ Уточню: **цены в каталоге не указаны**, поэтому я не могу подтвердить, укладываются ли эти модели в ваш бюджет. Рекомендую уточнить стоимость у менеджера магазина.

**AI честно отказался выдумывать** — хотя в реальности 3 из 4 найденных товаров (5 690 ₽, 10 990 ₽, 12 490 ₽) укладываются в бюджет. Цены лежат в метаданных карточек, но **в текст для AI пока не попадают**. Это закрывается одной правкой в подготовке текста для AI (планируется на следующей неделе) — после этого AI начнёт сразу называть цену в ответе.

#### Q: «DWM-101S — какие картриджи и когда менять?»

> По данным каталога, информация о фильтре **DWM-101S** и его картриджах в доступном контексте отсутствует.
>
> Могу сказать, что мембранный модуль **КО-100S** подходит для моделей DWM-31, DWM-102S Морион, DWM-201 и DWM-202S — но **не для DWM-101S**.

**AI снова не выдумал** — корректно отметил что КО-100S **не для DWM-101S** (тонкая деталь, прочитал из описания самого модуля). Но конкретный список картриджей и срок их замены **не вернул**: в карточке DWM-101S либо не привязан System Bundle с расходниками, либо у самих картриджей не заполнен атрибут «Средний срок службы расходника, месяцев». Это **самый частый запрос клиентов после покупки** — закрывается через заполнение атрибутов картриджей и проверку System Bundle DWM-101S.

### Что это всё значит

**Качественная сторона:**

- **AI не выдумывает** — на 2 запросах из 7 он сказал «не знаю» вместо вранья. Это критически важно для бренда: лучше один раз направить клиента к менеджеру, чем продать неподходящее решение.
- **AI цитирует конкретные модели** — Pro B, Трио Fe H, Кристалл А, DWM-101S, Фаворит. Это работает как реклама внутри ответов — клиент сразу видит линейку Аквафор.
- **AI ведёт диалог консультанта** — задаёт уточняющие вопросы про use case (дом vs питьё), бюджет, тип воды. Не справочник, а собеседник.

**Финансовая сторона:**

- **Себестоимость 7 запросов: $0.04 ≈ 3 ₽** (по курсу 80 ₽/$). На клиента в среднем 5-10 запросов в диалоге = **2-4 ₽ на полный разговор**.
- В пересчёте на месяц при 100 клиентов × 5 запросов = 500 запросов ≈ **150 ₽/мес**. При 1000 клиентах = 1 500 ₽/мес. Это **меньше зарплаты одного менеджера за один рабочий день**, при том что AI обрабатывает 24/7.
- ROI: один отвлечённый от типовых вопросов менеджер (~50 000 ₽/мес) окупает AI-консультанта в **300+ раз**.

**Что даёт заполнение карточек по этому гайду:**

| Сценарий | Сейчас (155 карточек как есть) | После заполнения по гайду |
|---|---|---|
| «Бюджет до 15 000 ₽» | AI говорит «цен нет» | AI: «Кристалл А — 5 690 ₽, Фаворит — 10 990 ₽» (требует backend-правки + цены в карточках) |
| «Какие картриджи DWM-101S» | «информация отсутствует» | AI: «Pro 1, Pro H, Pro B — менять каждые 12 месяцев» (требует System Bundle + атрибут «срок службы» на картриджах) |
| «Жёсткая вода 12 мг-экв/л» | AI говорит «модели рассчитаны до 5» | AI категорически отбрасывает товары с `max_hardness ≤ 5` и предлагает обратный осмос (требует атрибут пределов очистки) |
| «Похожее но дешевле» | AI не сравнивает по цене | AI: «Кристалл H ECO дешевле Кристалл H Pro на 4 000 ₽, ту же воду фильтрует» |
| Подбор по адресу (карта PROSTOR) | AI находит по описанию | AI отбрасывает не подходящие по пределам очистки **категорически** + предлагает комплект (предфильтр + основной) |

### Технический риск-чек

PoC прогнан на реальном продакшен-каталоге, не на тестовых данных. Артефакты воспроизводимы:

- 7 запросов с источниками: `experiments/poc-catalog-qa/results.json` (155 товаров × 682 чанка в Document Store).
- Время ответа: 4-7 секунд на запрос (приемлемо для диалога).
- Стек: Claude Sonnet 4.6 (LLM) + OpenAI `text-embedding-3-large` (semantic поиск) + Postgres pgvector (хранилище эмбеддингов) + Flowise (RAG-оркестратор) — всё уже в продакшене slovo для smart-search Phase 1+2.
- Подключение AI-консультанта к существующему каталогу — **отдельный chatflow за полдня работы**, не новая инфраструктура.

### Куда это пойдёт в PROSTOR

AI-консультант запланирован как **sticky chat-panel в углу страницы каталога** (отдельная feature, отдельный UX от smart-search). Не на главной (преждевременно в воронке), не на карте PROSTOR (там другой UX — анализ воды по адресу). Позиционирование: **первая линия 24/7 + квалификация лида** — 70% типовых вопросов AI закрывает сам, 30% сложных передаёт менеджеру с готовым контекстом диалога. План-stub: `docs/features/catalog-ai-consultant.md` (2026-05-19, ждёт триггера запуска — Phase 1.5 closure + ≥30 карточек заполнено по этому гайду).
...
|---|---|---|---|
| Описание / название / текст карточки → embedding | slovo | ✅ работает | с 2026-05-02 (vision-catalog Phase 1) |
...
В приложении PROSTOR (карта качества воды + каталог) работают **две** функции, питающиеся из карточек МС:
...
### Важно: новые атрибуты МС автоматически попадают в embedding
...
Из секции «Как карточка превращается в embedding» видно: feeder собирает текст из **пяти параллельных зон**, каждая попадает в свою часть embedding'a. Менеджер заполняет **все зоны**, у каждой своя роль. Нет выбора «или текст или атрибуты» — обе работают.
...
**Что заполнять:** числовые/справочные/boolean атрибуты карточки. **Любые** атрибуты МС автоматически попадают в embedding (кроме 4 служебных), без работы программистов — см. блок «Важно: автоматический подхват» в секции embedding format выше.
...
### Шаг 1. Кастомные атрибуты МС (5-15 полей в зависимости от категории)
...
Это **те же 16 параметров СанПиН (17 атрибутов: pH разнесён на min/max) 1.2.3685-21**, что используются в реальных водо-анализах (отчёты на сайте/в приложении Аквафор-Pro показывают эти параметры). AI сравнивает прогноз/реальный анализ воды у клиента с пределами каждого фильтра.
...
| ➕ `removes_mechanical` | да/нет | да | удаляет механические примеси (для предфильтров) |
| ➕ `removes_iron` | да/нет | да (для Fe-картриджа) | специфическое удаление железа на предфильтре |
| ➕ `removes_softening` | да/нет | да (для Кристалл H с КН-картриджем) | умягчает воду частично (для проточных систем с КН) |
| ➕ `softening_efficiency_pct` | число | 95 (RO) / 70 (умягчающий картридж) | % удаления солей жёсткости (для оценки выходного качества) |

#### Не-СанПиН технические атрибуты (характеристики устройства)

| Атрибут МС | Тип | Пример | Зачем |
|---|---|---|---|
| ➕ `max_temperature_c` | число | 38 | макс. температура воды (для предфильтров и картриджей) |
| ➕ `max_pressure_bar` | число | 8 | макс. давление (для предфильтров и магистральных корпусов) |
| ➕ `mount_location` | справочник | «стояк» / «под мойку» / «магистраль» | где монтируется (для предфильтров) |
| ➕ `productivity_liters_per_day` | число | 145 | производительность RO-систем (литров в сутки) |
| ➕ `productivity_liters_per_hour` | число | 2 | производительность проточных (литров в час) |
| ➕ `has_storage_tank` | да/нет | да | есть ли накопительный бак |
| ➕ `storage_tank_volume_liters` | число | 5 | объём накопительного бака если есть |
...
| ➕ `cartridge_standard` | справочник | «DWM» / «Кристалл» / «Фаворит» / «10″ Slim Line» / «BigBlue 10″» / «BigBlue 20″» | совместимость с конкретным корпусом фильтра |
...
1. AI прогнозирует или знает реальный состав воды у клиента: например, `hardness_total: 12 мг-экв/л`, `iron_total: 0.4 мг/л`, `manganese_total: 0.2 мг/л`.
2. Для каждого товара проверяет: `max_hardness_mg_eq_l >= 12 AND max_iron_mg_l >= 0.4 AND max_manganese_mg_l >= 0.2 AND ...` (по всем 17 параметрам).
...
| `iron_total: 0.4` | ❌ (макс. 0.3) | ❌ (макс. 0.3) | ✓ (макс. 0.5) |
...
- **Никаких водяных знаков, логотипов чужих сайтов, фотографий товара с маркетплейсов.** При модерации Vision может расценить «www.competitor.com» на фото как product feature.
...
**[✅ Уже работает в production]** — даёт немедленный эффект на AI-поиск:
...
**Минимум для немедленной публикации:** все пункты группы **[✅ Уже работает]**. Это даёт работающий AI-поиск по описанию и фото плюс корректное отображение расходников и услуг в приложении PROSTOR. Группа **[➕ Желательные новые атрибуты]** работает семантически с первого refresh, а после deploy Phase 1.5 slovo дополнительно начнёт отбрасывать товары не подходящие по реальной воде клиента (например, кувшин при жёсткости >7).
...
- Архитектурное решение catalog-ingest: `docs/architecture/decisions/007-catalog-ingest-via-minio.md` (slovo)
...
**Версия документа:** 1.7 (2026-05-19)
