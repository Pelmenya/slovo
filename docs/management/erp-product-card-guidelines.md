...
2. **Существующие атрибуты МС** («Первый-Пятый элемент для обслуживания», «Первая-Третья услуга для обслуживания», «Электронный гарантийный талон», «Средний срок службы расходника, месяцев») — **уже используются** другими функциями приложения PROSTOR (списки услуг, расходников, гарантийные обязательства). Заполнение даёт пользу сразу.
3. **Новые предлагаемые атрибуты** (`max_*`, `aquaphor_filter_type` и др.) — заполнение **впрок** для дополнительной фильтрации по пределам воды клиента. Через 1-2 недели после backend Phase 1.5 — AI начнёт **отбрасывать товары не подходящие по реальной воде** (например, не предлагать кувшин когда у клиента жёсткость 12 мг-экв/л).
4. Менеджер не теряет время — описание/фото и существующие атрибуты дают эффект сразу, новые атрибуты повысят точность подбора когда backend подключит фильтрацию.
...
Плюс правильная категория МС и заполненные существующие атрибуты МС («...элемент для обслуживания», «...услуга для обслуживания», «Средний срок службы расходника», «Электронный гарантийный талон») — имена услуг и расходников **тоже обогащают семантический поиск** (по запросу «фильтр с установкой» AI находит товары с привязанной услугой «Установка смесителя 2-в-1»). Плюс они используются другими функциями приложения PROSTOR (карточки расходников, гарантийные обязательства).
...
Эти атрибуты **уже используются** другими функциями приложения PROSTOR (карточки расходников в приложении, AI подбор bundled услуг, гарантийные обязательства). Точные имена синхронизированы с кодом feeder'а Аквафор-CRM.

**На карточке фильтра:**

| Атрибут МС | Тип | Пример |
|---|---|---|
| ✅ «Первый элемент для обслуживания» | ссылка на товар-картридж | Модуль сменный К5 |
| ✅ «Второй элемент для обслуживания» | ссылка на товар-картридж | Модуль сменный К2 |
| ✅ «Третий элемент для обслуживания» | ссылка на товар-картридж | Модуль сменный мембранный КО-50S |
| ✅ «Четвёртый элемент для обслуживания» | ссылка на товар-картридж | Модуль сменный К7 |
| ✅ «Пятый элемент для обслуживания» | ссылка на товар-картридж | (пусто, если 4 ступени) |
| ✅ «Первая услуга для обслуживания» | ссылка на товар-услугу | Установка смесителя 2-в-1 |
| ✅ «Вторая услуга для обслуживания» | ссылка на товар-услугу | Замена фильтрующего модуля |
| ✅ «Третья услуга для обслуживания» | ссылка на товар-услугу | Диагностика фильтра |
| ✅ «Электронный гарантийный талон» | boolean | да (для основных фильтров с серийным номером) / нет (для аксессуаров) |

**На карточке картриджа:**

| Атрибут МС | Тип | Пример |
|---|---|---|
| ✅ «Средний срок службы расходника, месяцев» | число | 6 (К5) / 12 (К2) / 24 (мембрана) |
...
- ✅ «Средний срок службы расходника, месяцев» — числовой (например 6 для К5, 24 для мембраны КО-50S). Указывается **базовый срок для семьи из 4 человек** — другие функции PROSTOR корректируют по фактическому количеству жильцов автоматически.
...
### Услуги и расходники тоже работают как семантические якоря

Названия услуг и расходников **попадают в текст embedding'а товара** наряду с его описанием. Это значит:

- Когда клиент пишет в smart-search «фильтр с установкой под мойку» → AI находит товары у которых в «Первая услуга для обслуживания» привязана услуга «Установка смесителя 2-в-1 Аквафор» (имя услуги совпало с запросом семантически).
- Когда клиент пишет «фильтр с угольным картриджем» → AI находит товары у которых в «Первый-Пятый элемент» привязан «Модуль сменный К2 (угольная очистка)».

Поэтому **унифицированные понятные имена услуг и расходников** в МС напрямую повышают точность AI-поиска. Если в МС есть две услуги «Установка под мойку Аквафор» и «Монтаж под раковину» как **разные товары-услуги** — это две разных позиции в embedding'е, AI воспринимает их как разные понятия. Унификация в одну услугу с понятным названием → AI лучше связывает запрос клиента с товаром.
...
2. **Заполнить 5 атрибутов «...элемент для обслуживания»** — перечислить картриджи в порядке от первой ступени к последней. Это работает **прямо сейчас** в приложении PROSTOR (карточка фильтра показывает связанные расходники).
...
2. **Все картриджи имеют «Средний срок службы».** Без этого другие функции PROSTOR (расчёт замен) не сработают. Особенно для новых поступлений в каталог.
...
- [ ] ✅ «Первый-Пятый элемент для обслуживания» (для фильтров) — перечислены все картриджи в порядке от первой ступени к последней
...
**[🟡 Заработает в Phase 1.5 — недели]** — дополнительная фильтрация по пределам воды клиента (без этих атрибутов AI всё равно работает, но не отбрасывает неподходящие товары по жёсткости/железу/и т.д.):
...
Эти атрибуты **уже используются** другими функциями приложения PROSTOR (списки расходников и услуг в карточке товара, расчёт сроков замены, гарантийные обязательства). Заполнение даёт пользу сразу.
