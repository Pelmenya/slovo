...
2. **Качественные фото в нескольких ракурсах** — общий вид + товар в установленном состоянии + детали. AI описывает каждое фото в текст и использует это для поиска по запросу клиента и для smart-search «Сфоткай — найдём». Однотипные studio shots на белом фоне → AI не сможет связать клиентское фото из кухни с товаром.
...
Сейчас AI не знает «до какой жёсткости» работает каждый фильтр. Поэтому в рекомендации могут попадать неподходящие товары. После заполнения по этому руководству — AI будет фильтровать по пределам.

### 2. Умный поиск «Сфоткай — найдём» (smart-search)

Клиент фотографирует фильтр на полке (свой или у соседа) или вводит «фильтр под мойку как у мамы» → AI распознаёт категорию и подбирает похожие. **Пример:** клиент сфотографировал 3-колбовую систему Кристалл H Pro → AI должен предложить аналогичные проточные системы, а не системы обратного осмоса (визуально похожи, но это разные категории очистки).

Smart-search работает через **два слоя**:

1. **Vision-augmenter на ingest** — на этапе загрузки карточки в AI, Claude Vision (Haiku 4.5) описывает фото товара в текст («компактный белый фильтр под мойку с двумя прозрачными цилиндрическими колбами, накопительным баком слева, хромированным изогнутым краном»). Это описание добавляется в семантический индекс — товар становится «findable» по визуальным признакам даже если в текстовом описании их нет.
2. **Vision при поиске** — когда клиент в приложении загружает фото, Vision описывает его таким же образом, и AI ищет ближайшие совпадения по описаниям из каталога.

**Чтобы оба слоя работали** — фото в карточке МС должны быть хорошими (см. раздел «Фото товара» ниже). Плохие фото = неточный augmented description = товар не находится в smart-search даже когда клиент его явно ищет.

Сейчас AI путается потому что в описаниях нет явной категории и пределов. После заполнения — категория и пределы будут embedded в семантический индекс товара.

### 3. Автоматические уведомления о замене картриджей

После покупки фильтра (например, DWM-101S) backend знает: к этой модели прикреплены 4 картриджа (К5, К2, КО-50S, К7) с разными сроками службы (6 / 12 / 24 / 12 месяцев). Через 6 месяцев клиенту приходит push/email: «Пора менять К5 в вашем DWM-101S — стоимость 590 ₽, заказать в 1 клик».
...
Без этих атрибутов backend не сможет построить расписание уведомлений, а клиент забудет про замену — фильтр будет работать всухую.
...
flowchart LR
    ...
    D --> I[Backend schedule<br/>build<br/>cartridge_modules<br/>× service_life_months]
    I --> J[Push / email<br/>уведомления<br/>о замене<br/>картриджей]
```
...
- [ ] `service_life_months` (для картриджей) — заполнен, иначе backend не уведомит клиента о замене
...
- [ ] Карточка лежит в правильной группе МС (для автоматической привязки услуг/расходников)
- [ ] В описании есть секция «Что удаляет» с конкретным списком (не «всё», не «все примеси»)
- [ ] В описании есть секция «Что НЕ удаляет» если применимо (важно для honesty)
- [ ] В описании есть секция «Контекст использования» (для кого, при каких условиях)
- [ ] **Минимум 2-3 фото** в **разных ракурсах** (общий вид + товар в установленном состоянии + детали). Все ≥800×800, ≤5 МБ, JPEG/PNG/WebP, без водяных знаков и логотипов маркетплейсов
- [ ] Категория в МС соответствует физическому типу товара (не «Дополнительно» если это фильтр)

При выполнении всех пунктов — карточка готова к публикации.
