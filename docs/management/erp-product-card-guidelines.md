...
>
> **Версия документа:** 1.0 (2026-05-19). Это **целевое состояние** карточки. Часть атрибутов уже учитывается AI прямо сейчас (метка `[работает]` ниже), часть — будет учитываться после backend-обновления Phase 1.5 (метка `[скоро]`), часть — в плане Phase 2 (метка `[план]`). Менеджер может **заполнять впрок** — backend будет догонять.

---

## Состояние backend (на 2026-05-19)

| Что | Статус | Когда заработает |
|---|---|---|
| Описание / название / `pageContent` → embedding | ✅ работает | с 2026-05-02 (vision-catalog Phase 1) |
| Фото → Vision-augmenter описывает в текст → embedding | ✅ работает | с 2026-05-02 |
| 3 режима умного поиска (текст / фото / гибрид) | ✅ работает | с 2026-05-15 (smart-search Phase 1) |
| `matchScore: 0..100` per товар | ✅ работает | с 2026-05-15 |
| Dedupe chunks → unique products | ✅ работает | с 2026-05-18 (Phase 1.5 Slice 1) |
| `aquaphor_filter_type` enum в metadata + AI учитывает категорию | 🟡 скоро | Phase 1.5 Slice 2 (план: неделя) |
| Category-aware re-ranking (фото RO → топ-3 RO, не проточные) | 🟡 скоро | Phase 1.5 Slice 3 (после Slice 2) |
| `max_*` атрибуты → AI отбрасывает товары не подходящие по воде | 🟡 скоро | Phase 1.5 Slice 6 Step 2-3 (после parser) |
| Bbox image overlay (annotated rectangles на фото) | 🟡 скоро | Phase 1.5 Slice 4 (decision gate — Haiku precision) |
| Bundled services «Монтаж 2 500 ₽» в product card | 🟡 скоро | Phase 1.5 Slice 5 (зависит от MoySklad audit) |
| `cartridge_modules` + `service_life_months` → push-уведомления о замене | 🔵 план | Phase 2 (notifications module не запланирован) |
| Equipment-suggest filter по `specs.max*` | 🟡 скоро | Phase 1.5 Slice 6 Step 3 |

**Что это значит для менеджера:**

1. **Текстовое описание и фото** — заполнение даёт **немедленный эффект** в умном поиске и подборе.
2. **Атрибуты `max_*`, `aquaphor_filter_type`, `cartridge_modules`** — заполнение **впрок**. Через 1-2 недели после backend Phase 1.5 — атрибуты заработают для подбора по карте, smart-search и (позже) уведомлений.
3. Менеджер не теряет время — backend будет ждать его данных, а не наоборот.
...
2. **Качественные фото в нескольких ракурсах** — общий вид + товар в установленном состоянии + детали. AI описывает каждое фото в текст и использует это для всех 3 режимов умного поиска: только текст / только фото / гибрид (текст + фото). Однотипные studio shots на белом фоне → AI не сможет связать клиентское фото из кухни с товаром, и режим «гибрид» (самый мощный) не сработает.
...
4. **Расходники (`cartridge_modules`) на карточке фильтра** + **`service_life_months` на каждом картридже** — backend строит расписание уведомлений «пора менять К5» через 6 месяцев после покупки.
...
### 3. Автоматические уведомления о замене картриджей

После покупки фильтра (например, DWM-101S) backend будет знать: к этой модели прикреплены 4 картриджа (К5, К2, КО-50S, К7) с разными сроками службы (6 / 12 / 24 / 12 месяцев). Через 6 месяцев клиенту придёт push/email: «Пора менять К5 в вашем DWM-101S — стоимость 590 ₽, заказать в 1 клик».

**Статус: [план]** — Phase 2, notifications module ещё не разработан. Метаданные `cartridge_modules` и `service_life_months` можно собирать **впрок** — когда модуль notifications будет реализован, расписание построится автоматически из этих атрибутов.

Для этого нужны **две** вещи в МС:

1. На карточке фильтра — атрибут-связь «применяемые расходники» с перечнем картриджей (multi-ref на товары категории `replacement_module`).
2. На карточке каждого картриджа — атрибут `service_life_months` (срок службы в месяцах).

Без этих атрибутов backend не сможет построить расписание уведомлений (когда notifications module будет реализован), а клиент забудет про замену — фильтр будет работать всухую.
...
#### Базовая категоризация (обязательно для всех 5 категорий товаров)

| Атрибут МС | Тип | Пример | Зачем AI |
|---|---|---|---|
| `aquaphor_filter_type` | справочник | «обратный осмос» | основная категория для AI |
| `stages_count` | число | 4 | количество ступеней очистки (для фильтров) |
...
| `cartridge_modules` | multi-ref (товары) | К5, К2, КО-50S, К7 | картриджи в комплекте — backend строит расписание замен |
...
| `replacement_module` (картриджи) | `cartridge_standard`, `cartridge_function`, `service_life_months`, плюс пределы той функции, которую выполняет | — | — |
...
|---|---|---|---|
| `aquaphor_filter_type` | справочник | «сменный картридж» | категория |
...
| `service_life_months` | число | 6 | **критично:** срок службы — backend без этого не уведомит клиента о замене |
| `regenerable` | да/нет | да | регенерируемый (для умягчителей) |

Плюс — те `max_*` атрибуты из общей таблицы, которые соответствуют функции картриджа: для механического — `max_turbidity_emf`, `max_pressure_bar`; для умягчающего — `max_hardness_mg_eq_l`; для Fe-картриджа — `max_iron_mg_l`, `max_manganese_mg_l`.

**`service_life_months` — обязательный для всех картриджей.** Backend читает этот атрибут при формировании расписания уведомлений: «через 6 месяцев напомнить клиенту заменить К5 в его DWM-101S». Без атрибута уведомления не уйдут.
...
## Услуги и расходники — где привязывать

Услуги и расходники работают через **два разных уровня** привязки. Менеджер должен понимать оба, чтобы AI и backend получили правильные данные.

### Услуги (монтаж, сервис) → group-level

Услуги (монтаж под мойку, сервисное обслуживание, диагностика) одинаковые для **всей группы товаров** в МС. К каждой из 5 групп («Обратноосмотические системы», «Проточные системы», «Предфильтры», «Сменные модули», «Дополнительно») администратор МС один раз привязывает применимые услуги. После этого все товары в группе автоматически получают эти связи.

Менеджер **не привязывает** «Установка под мойку — 2 500 ₽» к каждой из 30 RO-систем — достаточно один раз на группе.

### Расходники (картриджи) → product-level

Расходники у каждого фильтра **разные**, даже внутри одной группы. У DWM-101S три картриджа (К5, К2, КО-50S, К7), у Аквафор Гросс Миди — другой набор (ЭФГ 112/250, B520-04, и т.д.). Поэтому расходники привязываются **на карточку конкретного фильтра** через атрибут-связь `cartridge_modules`.

```
Аквафор DWM-101S
Атрибут cartridge_modules (multi-ref на товары):
- Модуль сменный К5 (механическая предочистка)
- Модуль сменный К2 (угольная предочистка)
- Модуль сменный мембранный КО-50S (мембрана)
- Модуль сменный К7 (финишная очистка)

Аквафор Гросс Миди (10″)
Атрибут cartridge_modules:
- ЭФГ 112/250 — 10 мкм (механика)
- B520-04 (умягчающий)
- Fe-картридж (если установлен)
```

На карточке **каждого картриджа** должен быть атрибут `service_life_months` — backend без него не построит расписание уведомлений.

### Что фактически приходит в AI

В выгрузке `latest.json` для каждой карточки есть два массива:
...
```json
{
  ...
  "relatedServices": [
    ...
    { "id": "...", "name": "Диагностика фильтра", "kind": "maintenance" }
  ],
  "relatedComponents": [
    ...
    { "id": "card-k7-id", "name": "Модуль сменный К7" }
  ]
}
```

`relatedServices` приходит из группы, `relatedComponents` — из атрибута карточки.

### Что менеджеру делать в МС

1. **Положить карточку фильтра в правильную группу.** «Аквафор DWM-101S» должна быть в группе «Фильтры с краном / Обратноосмотические системы», а не в «Дополнительно». Иначе AI получит не те услуги.
2. **Заполнить атрибут `cartridge_modules` на карточке фильтра** — перечислить картриджи, которые в комплекте идут с фильтром и подлежат регулярной замене. Это multi-ref на товары из категории `replacement_module` (картриджи, у которых заполнен `service_life_months`).
3. **На карточке каждого картриджа** — обязательно заполнить `service_life_months` (например, 6 для К5 / 12 для К2 / 24 для мембраны КО-50S). Без этого backend не сможет уведомить клиента.
4. **Цены услуг заполнены** в МС в карточках самих услуг (не в карточках фильтров). Без цены приложение покажет «Монтаж — уточняйте», что снижает CR.

### Что улучшить (для администратора МС)

1. **Унификация наименования услуг.** Если в МС есть две позиции «Установка под мойку Аквафор» и «Монтаж под раковину» — AI воспринимает их как разные услуги (разные id). Унифицировать в одну.
2. **Покрытие групп услугами.** Проверить что **все 5 категорий** имеют привязанные применимые услуги. Аксессуары обычно без отдельной услуги монтажа — это нормально.
3. **Расходники во всех картриджах имеют `service_life_months`.** Это нужно для уведомлений о замене.
4. **Группировка картриджей по совместимости.** Картриджи в МС должны быть в подкатегории, соответствующей системе (например, «Сменные модули / Модули для DWM/Кристалл»). Тогда AI в фильтре по подкатегории покажет правильный набор.

### В описании товара про услуги/расходники писать НЕ нужно

Поскольку `relatedServices` и `relatedComponents` приходят в AI отдельно — **дублировать их в тексте описания не нужно**. Это создаёт два источника правды, один из которых неизбежно устареет.

**Исключение** — особенности конкретной модели, которые отличают её от группы. Например, «эта модель монтируется только на стояк ХВС, не на горячую» — особенность отдельного товара, имеет смысл в описании.
...
2. **Разрешение ≥ 800×800** пикселей. Меньше — Vision хуже распознаёт детали (колбы, индикаторы, маркировка).
3. **Формат JPEG / PNG / WebP.** Не SVG, не GIF, не TIFF.
4. **Размер файла ≤ 5 МБ** на фото.
...
1. **Общий вид на белом/нейтральном фоне** (1 шт) — для каталога и маркетплейсов. Это «продающее» фото.
2. **Товар в установленном состоянии** (1-2 шт) — фильтр под мойкой, предфильтр на стояке, картридж в корпусе. Это **самое важное** для smart-search: клиент фотографирует свой фильтр в кухонной обстановке → AI ищет похожие фото в каталоге. Если в каталоге только studio shots на белом фоне — AI не свяжет клиентское фото с товаром.
3. **Детали** (1-2 шт) — картриджи отдельно с маркировкой, кран чистой воды крупным планом, накопительный бак, индикаторы.
...
5. **В работе/в эксплуатации** (опционально, но мощно) — рука открывает кран, льётся вода, индикатор замены картриджа загорелся. Это даёт AI контекст «состояние», что повышает релевантность для запросов типа «фильтр у которого индикатор замены загорелся».
...
- **Backend через 6 месяцев после покупки** отправляет клиенту уведомление: «Пора менять К5 в вашем DWM-101S — заказать в 1 клик за 590 ₽» (читая `cartridge_modules` + `service_life_months: 6` у К5)
...
flowchart LR
    ...
    I -.-> H
    D -.-> J[Backend schedule<br/>cartridge_modules<br/>× service_life_months<br/>Phase 2 — план]
    J -.-> K[Push / email<br/>уведомления<br/>о замене<br/>Phase 2 — план]
    ...
    classDef phase2 fill:#cce5ff,stroke:#007bff,color:#004085,stroke-dasharray: 5 5
    class A,B,C,D,E,F,G,H phase1
    class I phase15
    class J,K phase2
```

Сплошные стрелки = **работает сейчас** (Phase 1 закрытый pipeline). Пунктирные с жёлтой заливкой = **в работе** (Phase 1.5 — недели). Пунктирные с синей заливкой = **план** (Phase 2 — кварталы).
...
- [ ] Минимум 2-3 фото в разных ракурсах (см. секцию «Фото товара») — Vision-augmenter обработает на следующем refresh
- [ ] Карточка лежит в правильной группе МС — услуги (relatedServices) подтягиваются автоматически
- [ ] Категория в МС соответствует физическому типу товара (не «Дополнительно» если это фильтр)

**[Скоро заработает — Phase 1.5]** — заполнение впрок, AI будет учитывать через 1-2 недели:

- [ ] `aquaphor_filter_type` атрибут выбран из 6 значений
- [ ] `stages_count` (для фильтров) и `housing_standard` (для совместимости с картриджами) заполнены
- [ ] `cartridge_modules` (для фильтров) — перечислены все картриджи, которые подлежат регулярной замене
...
**[Запланировано — Phase 2]** — для будущего notifications-модуля:

- [ ] `service_life_months` (для картриджей) — заполнен, чтобы backend в будущем построил расписание уведомлений

**Минимум для немедленной публикации:** все пункты группы **[Сразу работает]**. Группы **[Скоро]** и **[План]** заполняются впрок — backend подхватит атрибуты по мере выкатки соответствующих фич.
...
- **Услуги (монтаж, сервис, диагностика)** — на **группу в МС**. Разовая работа администратора МС: к каждой из 5 групп («Обратноосмотические системы», «Проточные системы», «Предфильтры», «Сменные модули», «Дополнительно») один раз привязываются применимые услуги. После этого все товары в группе получают связи автоматически.
- **Расходники (картриджи)** — на **карточку каждого фильтра** через атрибут `cartridge_modules` (multi-ref на товары-картриджи). Это потому что у DWM-101S свой набор картриджей (К5, К2, КО-50S, К7), у Аквафор Гросс — свой (ЭФГ, B520-04, Fe), даже если они в одной категории.

Главное: если положили карточку в правильную группу + заполнили `cartridge_modules` — всё подтянется автоматически. Если в неправильную (например, RO-систему в «Дополнительно») — AI получит не те услуги, а backend не построит расписание замен картриджей.

**Q: А зачем backend ещё и расписание замен строит? У клиента есть напоминалка в телефоне.**

A: Большинство клиентов забывают про замену картриджей — фильтр работает в режиме «исчерпан, но воду пропускает» месяцами, что хуже чем без фильтра (накопленные загрязнения смываются обратно в воду). Автоматическое уведомление «через 6 месяцев пора менять К5» с кнопкой «Заказать в 1 клик» — это:
...
Без `service_life_months` на картридже и `cartridge_modules` на фильтре backend не знает что и когда напомнить → клиент забывает → продажи картриджей падают. Это самая «дешёвая» оптимизация — заполнить 2 атрибута, получить +30-50% к продажам картриджей через год.
