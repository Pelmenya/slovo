...
>
> **Что меняется:** сейчас карточки заполняются как есть; этот документ предлагает 3 подхода и рекомендует один из них (гибрид) с готовыми шаблонами.
>
> **Версия документа:** 1.1 (2026-05-19, синхронизировано с реальным состоянием CRM `crm-aqua-kinetics-back`).
>
...
| **Telegram-уведомления о замене картриджей** (через атрибуты МС «Первый-Пятый элемент для обслуживания» + «Средний срок службы расходника, месяцев», с поправкой на жильцов в доме) | **CRM** (`crm-aqua-kinetics-back/installed-equipment-reminder.service.ts`) | ✅ работает | production, Cron `0 12 * * *` ежедневно, 7 дней до даты замены |
| **Электронный гарантийный талон** (мастер заполняет ГТ с серийным номером при boolean=true) | **CRM** | ✅ работает | через атрибут МС «Электронный гарантийный талон» |
| `aquaphor_filter_type` enum + AI учитывает категорию | slovo (новый атрибут МС) | 🟡 скоро | Phase 1.5 Slice 2 (план: неделя) |
| Category-aware re-ranking (фото RO → топ-3 RO, не проточные) | slovo | 🟡 скоро | Phase 1.5 Slice 3 (после Slice 2) |
| `max_*` атрибуты → AI отбрасывает товары не подходящие по воде | slovo (новые атрибуты МС) | 🟡 скоро | Phase 1.5 Slice 6 Step 2-3 (после parser) |
...
1. **Текстовое описание и фото** — заполнение даёт **немедленный эффект** в умном поиске и подборе.
2. **Атрибуты «...элемент для обслуживания» и «Средний срок службы расходника»** — уже работают в production CRM для Telegram-уведомлений о замене. Заполненные карточки сразу начинают уведомлять клиентов.
3. **Новые предлагаемые атрибуты** (`max_*`, `aquaphor_filter_type` и др.) — заполнение **впрок**. Через 1-2 недели после backend Phase 1.5 — заработают для AI-подбора по карте и smart-search.
4. Менеджер не теряет время — для существующих атрибутов эффект мгновенный, для новых backend догонит.
...
Чтобы AI-поиск работал семантически точно (не предлагал кувшин для жёсткой воды свыше 7 мг-экв/л) и backend мог автоматически уведомлять клиентов о замене картриджей, в карточке товара должны быть пять вещей:

1. **Чёткое название и описание** — что это, для какой воды, под мойку или магистральный.
2. **Качественные фото:** основное — прозрачный PNG без фона (как сейчас в каталоге Аквафор-Pro, единый стиль), плюс желательно 2-3 дополнительных в разных ракурсах (товар в установленном состоянии, детали). AI описывает каждое фото в текст и использует это для всех 3 режимов умного поиска: только текст / только фото / гибрид (текст + фото).
...
4. **Расходники через атрибуты МС «Первый-Пятый элемент для обслуживания» на карточке фильтра** + **«Средний срок службы расходника, месяцев» на каждом картридже** — CRM строит расписание уведомлений «пора менять К5» через 6 месяцев после покупки (через `InstalledEquipmentReminderService`).
5. **Правильная группа МС** — услуги (монтаж, сервис) подтягиваются автоматически из группы.

Документ рассматривает три способа добавить структурированные данные в МС и рекомендует **гибрид** (критичные параметры и связи — в кастомных атрибутах МС, контекст и особенности — в тексте описания).
...
**Статус:** базовый подбор по тексту описания **[работает]** (семантический поиск по описанию + категории МС). Фильтрация по `max_*` атрибутам **[скоро]** (Phase 1.5 Slice 6 — недели). Менеджер заполняет атрибуты **впрок**, чтобы backend начал ими фильтровать сразу после deploy Phase 1.5.
...
### 3. Telegram-уведомления о замене картриджей

**Статус: ✅ работает в production CRM** (`crm-aqua-kinetics-back/src/modules/installed-equipment/installed-equipment-reminder.service.ts`).

После того как у клиента установлен фильтр (запись `InstalledEquipment` в CRM-базе создаётся при заказе с услугой монтажа), CRM знает связи: фильтр → 5 картриджей через атрибуты МС, каждый картридж имеет свой срок службы. Ежедневный Cron `0 12 * * *` (15:00 по Москве) проверяет: какие картриджи нужно менять в ближайшие 7 дней. Список собирается, группируется по клиенту → клиент получает в Telegram сообщение «Пора менять К5 в DWM-101S по адресу [адрес] — заменить в 1 клик».

**Какие атрибуты МС нужны** (точные имена — в коде `crm-aqua-kinetics-back/src/modules/moy-sklad/modules/product/helpers/parse-maintenance-attributes.ts`):

На карточке **фильтра** — 5 ссылочных атрибутов (заполняются ссылками на товары-картриджи):
...
Если у фильтра меньше 5 картриджей — оставшиеся атрибуты остаются пустыми, CRM пропускает их.

На карточке **каждого картриджа** — числовой атрибут:

- ✅ «Средний срок службы расходника, месяцев» — например `6` для К5, `12` для К2, `24` для мембраны КО-50S

**Важно: срок указан для семьи из 4 человек.** CRM автоматически корректирует срок по реальному количеству жильцов на объекте: `adjustedMonths = baseLifespanMonths × (4 / residents)`, с ограничениями min 1 месяц, max base × 2. То есть для семьи из 2 человек срок удваивается, из 6 — сокращается в 1.5 раза.

**Без этих атрибутов уведомления не уйдут**, клиент забудет про замену — фильтр будет работать с истёкшим картриджем (хуже чем без фильтра, так как накопленные загрязнения смываются обратно в воду).

---

## Три варианта подхода

Есть три способа добавить пределы очистки и параметры в карточки МС.

### Вариант A — всё в поле «Описание» (текстом)

Менеджер пишет в обычном поле «Описание» структурированный текст с заголовками-метками:

```
Тип системы: Обратный осмос под мойку
Пределы очистки:
- Жёсткость общая: до 7 мг-экв/л
- Железо общее: до 0.3 мг/л
- TDS (минерализация): до 1500 мг/л
- Хлориды: до 350 мг/л

Что удаляет: соли жёсткости (95%), железо, марганец, хлор, органику, бактерии
Не удаляет: газы (сероводород без отдельной ступени)

Монтажные услуги:
- Установка под мойку — 2 500 ₽
- Подключение к стояку — 1 200 ₽

Расходники (картриджи):
- Модуль сменный К5 — 590 ₽
- Модуль сменный КН — 1 390 ₽ (умягчающий)
```

**Плюсы:**
- Не требует изменений в настройках МС (новых атрибутов / справочников)
- Один менеджер один раз перепишет — структура свободная
- Старые карточки можно постепенно «дообогащать» без миграции
- Гибкость: легко добавить нестандартный комментарий («работает только с холодной водой»)

**Минусы:**
- **Менеджер должен соблюдать формат заголовков строго.** Если один напишет «Пределы очистки:», другой «Лимиты по параметрам:», третий «Максимальные значения:» — AI парсер не найдёт половину.
- Опечатки в значениях («7,0 мг-экв/л» vs «7.0 мг-экв/л» vs «7 мг/экв л») могут ломать парсинг.
- В МС поле «Описание» — это plain-text без подсветки структуры, ошибки легко не заметить.
- При продвижении товара на маркетплейсы (Озон, ВБ) — этот текст уходит как есть, формальные «Пределы очистки:» в карточке маркетплейса смотрятся коряво.

**Подходит когда:** карточек мало (~20-50), один-два менеджера, нет времени настраивать МС.

---

### Вариант B — кастомные атрибуты в МС (structured fields)

В МС для каждой категории товаров (RO, проточные, предфильтры, картриджи, аксессуары) добавляются **доп. атрибуты** с типом число / справочник / диапазон:

```
Атрибуты для категории «Фильтры с краном / Обратноосмотические системы»:
- Тип системы [справочник]: «обратный осмос»
- Макс. жёсткость, мг-экв/л [число]: 10
- Макс. железо, мг/л [число]: 0.5
- Макс. TDS, мг/л [число]: 2000
- Удаляет хлор [да/нет]: да
- Удаляет бактерии [да/нет]: да
- Удаляет газы [да/нет]: нет
- Степень очистки, % [число]: 95
```

Менеджер при создании карточки заполняет эти поля в отдельных формах, а не пишет текстом.

**Плюсы:**
- Типизация: «10» вместо «10 мг-экв/л» или «жёсткость до десяти». Парсер 100% надёжен.
- Валидация на стороне МС — нельзя ввести «жёсткость 100500» (out of range).
- На маркетплейсах эти атрибуты можно выгрузить как «характеристики» — пользу для обычного клиента
- В выгрузке для slovo (через CRM) данные идут как structured JSON, не нужны regex
- Для AI это **самый качественный** сигнал — каждое поле типизировано и нормализовано

**Минусы:**
- **Настройка МС:** атрибуты создаются администратором МС (на стороне Аквафор) — нужны 5-7 атрибутов на каждую из 5 категорий = 25-35 атрибутов всего. Это разовая работа ~2-4 часа администратора МС.
- **Миграция 155 существующих карточек:** менеджер должен открыть каждую и заполнить атрибуты вручную (155 × 5 полей = 775 заполнений). При темпе 30 сек/поле — это ~6-7 часов работы. Можно разделить между менеджерами.
- **Нестандартные случаи** (товар с особенностями) — атрибут «удаляет газы» = «нет», но в реальности «с отдельным картриджем удаляет» — придётся добавлять либо новый атрибут «удаляет газы с доп.картриджем», либо комментарий в Описание (что возвращает нас к Варианту A).
- Атрибуты в МС бывает сложно поддерживать при изменении номенклатуры (добавление нового параметра — нужно создать атрибут и заполнить во всех существующих карточках).

**Подходит когда:** карточек много (100+), 3+ менеджеров, есть administrator МС для разовой настройки, готовы вложить ~10 часов работы один раз.

---

### Вариант C — гибрид (рекомендуемый)

**Пределы очистки по всем 17 параметрам СанПиН и тип системы → в кастомных атрибутах МС** (6 обязательных + 11 желательных/опциональных на категорию фильтра — см. полный справочник ниже).
**Услуги, расходники → в группе МС** (автоматическая привязка, не дублируется в текст).
**Особенности, контекст использования, что НЕ удаляет → в Описании** (rich text).

```
Атрибуты МС (для категории «RO под мойку», обязательный минимум):
- Тип системы [справочник «aquaphor_filter_type»]: «обратный осмос»
- Кол-во ступеней [число]: 4
- Стандарт корпуса [справочник]: «DWM»
- Макс. жёсткость, мг-экв/л [число]: 12
- Макс. железо, мг/л [число]: 0.5
- Макс. марганец, мг/л [число]: 0.3
- Макс. TDS, мг/л [число]: 2000
- Макс. хлориды, мг/л [число]: 700
- Макс. нитраты, мг/л [число]: 90

(+ 11 опциональных по другим параметрам СанПиН — см. справочник ниже)

Группа МС:
- «Очистка воды / Аквафор / Фильтры с краном / Обратноосмотические системы»
- К группе один раз привязаны: монтажные услуги, сервисное обслуживание, расходники

Описание (text, free-form):
Что удаляет: хлор, тяжёлые металлы, нефтепродукты, органику, бактерии, цисты лямблий
Что НЕ удаляет: растворённые газы (сероводород, аммиак)
Особенности: компактный корпус, низкое давление работы, экономит до 9 тонн воды/год
Контекст: подходит для дач, квартир, частных домов с центральным водопроводом
```

**Плюсы:**
- AI получает **точные пределы** из атрибутов (надёжный фильтр для подбора).
- AI получает **семантический контекст** из описания (что удаляет, какие особенности, монтаж/расходники) — для семантического поиска по тексту запроса «фильтр без газов».
- На маркетплейсах атрибуты дают «характеристики», описание даёт «преимущества».
- Поддержание баланса: атрибуты — только то что критично для AI-фильтрации (~5 полей), описание — всё остальное.
- Старые карточки можно мигрировать постепенно — атрибуты заполнить за один проход (бизнес-приоритет), описание дополнить по мере touch'ей.

**Минусы:**
- Дублирование риск: одно и то же может быть и в атрибуте, и в тексте. Решается правилом «атрибут — источник правды, текст — пересказ для маркетплейса».
- Чуть больше «правил» для менеджера: понимать что куда писать. Решается шаблонами (ниже).

**Подходит когда:** боль смешана — много карточек (155+), нужна точность AI, но также важно качество текста для маркетплейсов. **Это наш случай.**
...
- **155 карточек × 5 атрибутов = 775 заполнений** — это ~6 часов работы при темпе 30 сек/поле, можно разделить между 3 менеджерами на 2 часа каждому. Окупается мгновенно через точность подбора.
- **Без атрибутов** (только Вариант A) — AI парсер будет ошибаться на ~15-20% карточек (typos в формате, забытые секции).
- **Только атрибуты** (Вариант B) — потеряем гибкость для нестандартных случаев + плохая UX на маркетплейсах (характеристики без описаний).
- Атрибуты для маркетплейсов всё равно нужны (Ozon / WB требуют structured данные) — обе цели совпадают.

Остаток документа описывает гибридный подход в деталях.

---

## Структура карточки (гибрид)
...
#### Существующие атрибуты МС (✅ работают в production)

Эти атрибуты **уже используются CRM** для Telegram-уведомлений о замене и заполнения гарантийных талонов. Точные имена синхронизированы с кодом `crm-aqua-kinetics-back/src/modules/moy-sklad/modules/product/helpers/parse-maintenance-attributes.ts`.

**На карточке фильтра:**

| Атрибут МС | Тип | Пример | Используется для |
|---|---|---|---|
| ✅ «Первый элемент для обслуживания» | ссылка на товар | Модуль сменный К5 | Telegram-уведомления о замене |
| ✅ «Второй элемент для обслуживания» | ссылка на товар | Модуль сменный К2 | Telegram-уведомления |
| ✅ «Третий элемент для обслуживания» | ссылка на товар | Модуль сменный мембранный КО-50S | Telegram-уведомления |
| ✅ «Четвёртый элемент для обслуживания» | ссылка на товар | Модуль сменный К7 | Telegram-уведомления |
| ✅ «Пятый элемент для обслуживания» | ссылка на товар | (пусто, если 4 ступени) | Telegram-уведомления |
| ✅ «Первая услуга для обслуживания» | ссылка на товар-услугу | Установка смесителя 2-в-1 | UI приложения PROSTOR, AI подбор bundled |
| ✅ «Вторая услуга для обслуживания» | ссылка на товар-услугу | Замена фильтрующего модуля | UI, AI |
| ✅ «Третья услуга для обслуживания» | ссылка на товар-услугу | Диагностика фильтра | UI, AI |
| ✅ «Электронный гарантийный талон» | boolean | да (для основных фильтров) / нет (для аксессуаров) | Мастер обязан заполнить ГТ с серийным номером при monтаже |

**На карточке картриджа:**

| Атрибут МС | Тип | Пример | Используется для |
|---|---|---|---|
| ✅ «Средний срок службы расходника, месяцев» | число | 6 (К5) / 12 (К2) / 24 (мембрана) | Расчёт даты замены в Telegram-уведомлении (с поправкой на жильцов) |
...
**«Средний срок службы расходника, месяцев» — обязательный для всех картриджей.** Этот атрибут уже работает в production CRM: ежедневный Cron находит картриджи у которых `nextReplacementDate ≤ now + 7 дней` (с поправкой на жильцов) и шлёт клиенту Telegram-уведомление «пора менять К5 в DWM-101S». Без атрибута уведомления не уйдут.
...
Услуги и расходники в МойСклад привязываются через **существующие атрибуты карточки фильтра** (не группы товаров, как я раньше предполагал). Реальная схема, которая работает в CRM сейчас:

### Атрибуты на карточке фильтра — 5 + 3 + 1 = 9 атрибутов

**5 ссылочных атрибутов на расходники** (✅ существуют в МС, ✅ используются CRM):
...
Заполняются по порядку первой ступени → последней. Лишние оставляются пустыми (CRM пропускает).

**3 ссылочных атрибута на услуги** (✅ существуют в МС, ✅ используются CRM):

- ✅ «Первая услуга для обслуживания» — ссылка на товар-услугу
- ✅ «Вторая услуга для обслуживания» — ссылка
- ✅ «Третья услуга для обслуживания» — ссылка

Обычно: первая — монтажная (установка под мойку), вторая — сервисная (замена картриджа), третья — диагностика. Имена услуг и цены прописаны в карточках самих услуг.

**1 boolean атрибут на гарантийный талон** (✅ существует в МС, ✅ используется CRM):

- ✅ «Электронный гарантийный талон» — да/нет

Если `true` — при завершении заказа на установку **мастер обязан заполнить ГТ с серийным номером** в приложении PROSTOR. Если `false` — гарантийный талон не требуется (для аксессуаров, расходников).

### Атрибут на карточке картриджа — 1 атрибут

- ✅ «Средний срок службы расходника, месяцев» — числовой (например 6 для К5, 24 для мембраны КО-50S)

**Срок указан для семьи из 4 человек.** CRM корректирует по реальному количеству жильцов: `adjustedMonths = baseLifespanMonths × (4 / residents)`. То есть для одинокого пользователя срок будет в 4 раза дольше, для семьи из 6 — короче в 1.5 раза. Без этого атрибута CRM не сможет уведомить о замене.

### Что фактически приходит в AI (slovo)

В выгрузке `latest.json` для каждого товара есть массивы:

```json
{
  "externalId": "d2c43c8c-cc04-...",
  "name": "Аквафор DWM-101S",
  ...
  "relatedServices": [
    { "id": "...", "name": "Установка смесителя 2-в-1 Аквафор", "kind": "installation" },
    { "id": "...", "name": "Замена фильтрующего модуля", "kind": "maintenance" }
  ],
  "relatedComponents": [
    { "id": "card-k5-id", "name": "Модуль сменный К5" },
    { "id": "card-k2-id", "name": "Модуль сменный К2" },
    { "id": "card-ko-50s-id", "name": "Модуль сменный мембранный КО-50S" }
  ]
}
```

`relatedServices` и `relatedComponents` собираются feeder'ом Аквафор (CRM) из существующих атрибутов МС («Первая-Третья услуга...» и «Первый-Пятый элемент...») и передаются в slovo для embedding и UI.

### Что менеджеру делать в МС

1. **Положить карточку фильтра в правильную категорию МС.** «Аквафор DWM-101S» в «Фильтры с краном / Обратноосмотические системы», не в «Дополнительно». Категория не используется напрямую как услуга, но важна для UI и для будущей AI-классификации.
2. **Заполнить 5 атрибутов «...элемент для обслуживания»** — перечислить картриджи в порядке от первой ступени к последней. Это работает **прямо сейчас** для Telegram-уведомлений о замене.
3. **Заполнить 3 атрибута «...услуга для обслуживания»** — перечислить применимые услуги (монтаж, сервис, диагностика). Это работает для отображения услуг в UI и для будущей AI-подбора bundled услуг.
4. **На карточке каждого картриджа** — обязательно заполнить «Средний срок службы расходника, месяцев». Без этого CRM не построит расписание замен.
5. **Boolean «Электронный гарантийный талон»** — `true` для фильтров где нужен ГТ (RO, проточные, предфильтры с серийным номером), `false` для accessories и расходников.
6. **Цены услуг** — заполнены в карточках самих услуг, не на карточке фильтра.

### Что улучшить (для администратора МС)

1. **Унификация наименования услуг.** Если в МС есть две позиции «Установка под мойку Аквафор» и «Монтаж под раковину» — это разные товары-услуги, AI и CRM воспринимают их как разные. Унифицировать в одну.
2. **Все картриджи имеют «Средний срок службы».** Без этого Telegram-уведомление не отправится. Особенно для новых поступлений в каталог.
3. **«Электронный гарантийный талон» выставлен правильно** для всех фильтров где нужно собирать серийные номера. Прямо влияет на сервисные обязательства.

### В описании товара про услуги/расходники писать НЕ нужно

Поскольку `relatedServices` и `relatedComponents` приходят в slovo отдельно из существующих атрибутов МС — **дублировать их в тексте описания не нужно**. Это создаёт два источника правды, один из которых неизбежно устареет.

**Исключение** — особенности конкретной модели, которые отличают её от похожих. Например, «эта модель монтируется только на стояк ХВС, не на горячую» — особенность отдельного товара, имеет смысл в описании.
...
Менеджер открывает карточку в МС, копирует шаблон ниже, заполняет конкретные значения. Заголовки секций — **обязательны** в том же виде (AI парсер их ищет).
...
Тип системы: обратный осмос
Количество ступеней: [N]
Производительность: [N] литров в сутки
Накопительный бак: [есть/нет, объём N л]

Что удаляет:
- Соли жёсткости (до 95%)
- Хлор и хлорорганика
- Железо до [N] мг/л
- Тяжёлые металлы (свинец, медь, никель)
- Бактерии, вирусы, цисты лямблий
...
Особенности (только если отличают от группы):
...
**Не дублировать в описании:** монтажные услуги, сервисное обслуживание, перечень картриджей — приходят автоматически из `relatedServices` / `relatedComponents` группы товара.
...
Тип системы: проточный фильтр
Количество ступеней: [N]
Производительность: [N] литров в час
Накопительный бак: нет (проточный режим)

Что удаляет:
- Хлор и хлорорганика
- Железо до [N] мг/л (если есть Fe-картридж)
- Тяжёлые металлы
- Песок, ржавчина, окалина
- [Если умягчающий — добавить «соли жёсткости частично, регенерируется солью»]

Что НЕ удаляет:
- Соли жёсткости свыше [N] мг-экв/л (для жёсткой воды — выбирать обратный осмос)
- Бактерии (для бактерицидной защиты — UV-лампа отдельно)
- Растворённые газы

Контекст использования:
Подходит для квартир с центральным водопроводом, где жёсткость не превышает 7 мг-экв/л и нет высокого железа. Альтернатива обратному осмосу при ограниченном бюджете и небольшом расходе воды.
...
**Не дублировать в описании:** монтажные услуги, сервисное обслуживание, перечень картриджей — приходят автоматически из связей группы товара.

### Шаблон для категории `pre_filter` (магистральный предфильтр)

```
Аквафор [МОДЕЛЬ] — магистральный предфильтр на стояк (или под мойку).

Тип системы: магистральный предфильтр
Стандарт корпуса: [10″ Slim Line / 10″ BigBlue / 20″ BigBlue / Гросс]
Точка установки: [стояк / только холодная / только горячая / магистраль перед бойлером]

Что удаляет:
- Песок, ржавчину, окалину, мусор
- [Если с Fe-картриджем — «железо растворённое до N мг/л»]
- [Если с угольным — «хлор и запах»]
...
Особенности:
- Корпус [пластиковый / металлический], выдерживает до [N] бар давления
- Картриджи стандарта [10″ Slim Line / BigBlue 10″ / BigBlue 20″] — широкая совместимость
```

**Не дублировать в описании:** монтажные услуги, сервисное обслуживание, перечень картриджей — приходят автоматически из связей группы товара.

### Шаблон для категории `replacement_module` (сменный модуль)

```
Сменный модуль Аквафор [НАЗВАНИЕ] — картридж для [СОВМЕСТИМОСТЬ].

Тип: сменный картридж
Совместимость: [DWM / Кристалл / Фаворит / Гросс / BigBlue 10″ / BigBlue 20″]
Функция: [механика / уголь / умягчение / железо / мембрана / комплект]

Что делает:
- [Конкретные параметры — пористость, состав засыпки, и т.п.]
- Ресурс: [N] литров или [N] месяцев (что наступит раньше)
- [Если регенерируемый — описать процесс]

Параметры работы:
- Макс. температура воды: [N]°C
- Макс. давление: [N] бар
- [Если требует холодную только — указать]

Когда менять:
[Конкретное описание индикаторов — изменение цвета, падение скорости фильтрации, по календарю]

Связанные системы (для которых подходит):
[Список конкретных моделей фильтров — DWM-101S, DWM-202S, Кристалл H Pro, ...]
```
...
Тип: аксессуар
Вид: [кран чистой воды / клапан защиты от протечек / трубка / фитинг / монтажный набор]

Назначение:
[Конкретное описание функции]

Совместимость:
[Список конкретных моделей фильтров — DWM-серия, Кристалл-серия, Фаворит, OSMO, ...]

Особенности:
- [Материал, размер, цвет, тип резьбы и т.п.]

Монтажные услуги:
[Если требуется отдельная услуга установки — описать; если ставится при основной установке фильтра — указать «входит в стандартный монтаж»]
```
...
Существующие атрибуты МС (✅ работают сейчас):
- ✅ «Первый элемент для обслуживания»: Модуль сменный К5 (механическая предочистка)
- ✅ «Второй элемент для обслуживания»: Модуль сменный К2 (угольная предочистка)
- ✅ «Третий элемент для обслуживания»: Модуль сменный мембранный КО-50S (мембрана)
- ✅ «Четвёртый элемент для обслуживания»: Модуль сменный К7 (финишная очистка)
- ✅ «Первая услуга для обслуживания»: Установка под мойку — 2 500 ₽
- ✅ «Вторая услуга для обслуживания»: Замена картриджей — от 1 500 ₽
- ✅ «Третья услуга для обслуживания»: Диагностика — от 800 ₽
- ✅ «Электронный гарантийный талон»: да (мастер заполняет ГТ при монтаже)
...
Тип системы: обратный осмос
Количество ступеней: 4 (К5 механика → К2 уголь → КО-50S мембрана → К7 финиш)
Производительность: 145 литров в сутки
Накопительный бак: встроенный, 5 л

Что удаляет:
- Соли жёсткости (до 95%)
- Хлор и хлорорганика
- Железо до 0.5 мг/л
- Тяжёлые металлы (свинец, медь, никель, кадмий)
- Бактерии, вирусы, цисты лямблий
- Нефтепродукты, пестициды

Что НЕ удаляет:
- Растворённые газы (сероводород, аммиак)
...
(Услуги и расходники приходят автоматически из группы товара в МС — в описании их не дублируем.)
...
- **CRM через 6 месяцев после установки** отправляет клиенту Telegram-уведомление: «Пора менять К5 в вашем DWM-101S — заказать в 1 клик за 590 ₽» (читая ✅ «Первый элемент для обслуживания» = К5 + ✅ «Средний срок службы расходника, месяцев» = 6 у К5, с поправкой на жильцов)
...
flowchart LR
    ...
    I -.-> H
    B --> M[CRM cron `0 12 * * *`<br/>InstalledEquipmentReminderService<br/>читает «Первый-Пятый элемент»<br/>+ «Средний срок службы расходника»]
    M --> N[Telegram bot<br/>уведомления<br/>«пора менять К5»]
    ...
    class A,B,C,D,E,F,G,H,M,N phase1
    class I phase15
```

Сплошные зелёные узлы = **работает сейчас** (Phase 1 закрытый pipeline + CRM Telegram уведомления через существующие атрибуты МС). Пунктирные с жёлтой заливкой = **в работе** (Phase 1.5 — недели, новые атрибуты МС для AI-фильтрации).
...
- [ ] ✅ «Первый-Пятый элемент для обслуживания» (для фильтров) — перечислены все картриджи в порядке от первой ступени к последней (работает в CRM для Telegram-уведомлений о замене)
- [ ] ✅ «Первая-Третья услуга для обслуживания» (для фильтров) — монтаж / сервис / диагностика
...
- [ ] ✅ «Средний срок службы расходника, месяцев» (для картриджей) — заполнен числом, **критично для CRM** (без него Telegram-уведомление не уйдёт)
- [ ] Карточка лежит в правильной категории МС (не «Дополнительно» если это фильтр)

**[🟡 Заработает в Phase 1.5 — недели]** — заполнение впрок, slovo AI будет учитывать после deploy:
...
**Минимум для немедленной публикации:** все пункты группы **[✅ Уже работает]**. Это даёт работающие Telegram-уведомления о замене, AI-поиск по описанию и фото. Группа **[🟡 Скоро]** заполняется впрок — slovo AI догонит после deploy Phase 1.5.
...
A: К **карточке каждого фильтра**, через существующие атрибуты МС (✅ работают сейчас):

- **Расходники** — атрибуты «Первый/Второй/Третий/Четвёртый/Пятый элемент для обслуживания» (5 ссылок на товары-картриджи в порядке от первой ступени).
- **Услуги** — атрибуты «Первая/Вторая/Третья услуга для обслуживания» (3 ссылки на товары-услуги).
- **Срок службы** каждого картриджа — в его собственной карточке через атрибут «Средний срок службы расходника, месяцев».

У DWM-101S — свой набор картриджей и услуг через эти 8 атрибутов, у Аквафор Гросс — свой. Это **product-level** связи, не группа товаров.

Если положили карточку в правильную категорию МС (а не «Дополнительно» если это RO-фильтр) + заполнили эти 8 атрибутов — Telegram-уведомления о замене и список услуг в приложении PROSTOR работают прямо сейчас.

**Q: А зачем CRM расписание замен строит? У клиента есть напоминалка в телефоне.**

A: Большинство клиентов забывают про замену картриджей — фильтр работает в режиме «исчерпан, но воду пропускает» месяцами, что хуже чем без фильтра (накопленные загрязнения смываются обратно в воду). Автоматическое Telegram-уведомление «через 6 месяцев пора менять К5» с кнопкой «Заказать в 1 клик» — это:

1. **Защита здоровья клиента** (по факту использования паспортного срока)
2. **Повторные продажи Аквафор** (картриджи — recurring revenue, до 60% от продажи фильтра в год)
3. **Лояльность к бренду** (клиент чувствует заботу, не получает «брошенный» продукт)

Pipeline работает (`crm-aqua-kinetics-back/src/modules/installed-equipment/installed-equipment-reminder.service.ts`, Cron `0 12 * * *`):

1. CRM ежедневно в 15:00 МСК проверяет таблицу `InstalledComponent` (картриджи установленные у клиентов)
2. Для каждого считает `nextReplacementDate` от `installedAt` + `adjustedLifespanMonths` (где `adjusted = base × 4/residents`, base из атрибута МС)
3. Если `nextReplacementDate ≤ now + 7 дней` и `isReplaced = false` — собирает в reminder
4. Группирует по telegramId клиента, отправляет через `BotService.sendReplacementReminder`

Без атрибутов «Первый-Пятый элемент для обслуживания» на карточке фильтра и «Средний срок службы расходника» на каждом картридже CRM не знает что и когда напомнить → клиент забывает → продажи картриджей падают. Это самая «дешёвая» оптимизация — заполнить существующие атрибуты, получить +30-50% к продажам картриджей через год.
