...
- **`@slovo/flowise-flowdata` factories пропускают `data.filePath` + `data.icon` + `loadMethods: {}` + `display: true`** — runtime срабатывает (prediction отвечает, retriever возвращает sources), но UI рендерит asyncOptions dropdowns как «No options» и не показывает Connect Credential. Backend `/api/v1/node-load-method/<nodeName>` без filePath не знает откуда импортировать node module → возвращает пустой массив. **Workaround** — post-processing `enrichNodeData(node, spec)` после `fromIntrospection`: добавляет `filePath`/`icon`/`loadMethods: {}` в `data`, копирует `loadMethod`/`default`/`additionalParams`/`options`/`show`/`hide`/`step`/`rows`/`description` из spec на каждый `inputParams[i]`, ставит `display: true` на все input/inputParams/inputAnchors. Эталонный пример — `experiments/poc-catalog-qa/build-flowdata.ts`. **Long-term** — поднять factories в `libs/flowise-flowdata/` так чтобы они автоматически генерили полный node-data через `fromIntrospection` (см. memory `feedback_flowise_flowdata_ui_render_requires_filePath`). Прецедент 2026-05-19 (catalog-qa-poc-v1): 7/7 prediction'ов прошли успешно, но в UI Model Name был пустым и Connect Credential не отображался → правомерное «модель не подключена» от менеджера.
