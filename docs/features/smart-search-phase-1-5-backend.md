...
### Slice 8 — Денормализация срока замены картриджей в карточку системы 🟡 Part B closed 2026-05-19, Part A pending
...
**Статус:** Часть B (CRM-side денормализация) **закрыта** 2026-05-19 ~16:07 МСК — закоммичено в crm-back (`3d27438` + `9943677` ё/е fix + `58940d0` reorder), real e2e proof в `docs/experiments/knowledge-base-poc/2026-05-19-catalog-qa-baseline.md` секция «Slice 8 implementation result». Q5 «DWM-101S — картриджи и сроки» возвращает **точную таблицу 6/6/24/12 мес**. 20-Q extended Q&A batch показал **20/20 success** на текущем каталоге. Часть A (slovo pre-expander) — pending до `catalog-ai-consultant` запуска.
