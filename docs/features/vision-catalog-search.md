...
| **PR6 prerequisite** | `FLOWISE_API_KEY` в `libs/common/src/config/env.schema.ts` — slovo runtime начнёт ходить в Flowise REST. Без этого 401-сюрпризы в проде. См. tech-debt секция C пункт 2 (поднят с in-PR6 до pre-PR6). | 🟡 первым |
| **PR6** — `apps/worker/catalog-refresh` | RMQ-consumer + cron `@nestjs/schedule` запускает refresh каждые 4ч. Логика **проста по форме, тонкая по нюансам**: `flowise_docstore_search_by_name` → `flowise_docstore_refresh`. Flowise S3 File Loader сам тянет `latest.json` из bucket'а (env в `docker-compose.infra.yml`, см. lab journal 12:00-12:30). **Тонкости из исходника Flowise (`dist/services/documentstore/index.js:1625` `refreshDocStoreMiddleware`):** (1) **Refresh синхронен** — HTTP request зависает до полного завершения upsert всех loader'ов (на 155 items ~5-30 сек, на 1000+ — минуты). (2) **Distributed lock через Redis обязателен** — если предыдущий cron-tick не завершился, новый запустит **второй** refresh параллельно. Используй `ioredis` SET NX EX из `libs/common` или RMQ unique-message constraint. (3) **`replaceExisting=true`** в payload `flowise_docstore_refresh` чтобы старые chunks удалялись (без него dangling vectors). См. `dist/services/documentstore/index.js:1364` `upsertDocStore.replaceExisting` — default false. **~120-150 LOC + spec'и** (был ~80, +40 на lock + retry + replaceExisting). | 🟡 следующий |
| **PR7** — `/catalog/search/text` | `apps/api/modules/catalog/search/text.controller.ts` + service. Один вызов `flowise_docstore_query({ storeId, query, topK })`. **Enrichment в slovo:** presigned URLs для `imageUrls[]` (S3-keys из metadata Document Store) через `libs/storage.getSignedUrl(key, { expiresIn: 3600 })` — TTL 1ч. **Кэширование presigned URLs**: на стороне slovo — Redis с TTL 50 мин (немного меньше чем сам signed URL чтобы не отдать просроченный). Parsing metadata: когда CRM feeder перейдёт на `application/json` без charset (lab journal day 2) — Flowise S3 File Loader включит JSONLoader и distinct metadata появится сама. До этого — fallback на парсинг pageContent (текущий blocked path). **~150 LOC + spec'и + e2e** (был ~120, +30 на cache layer). | 🟡 после PR6 |
...
1. ✅ **Flowise Vector Store под капотом — ЗАКРЫТ (2026-04-29).** Flowise создаёт таблицу с именем из `Table Name` поля Postgres-ноды (у нас `catalog_chunks`), не `langchain_pg_embedding`. Schema стандартная: `id (uuid) / pageContent (text) / metadata (jsonb) / embedding (vector)`. HNSW-индекс — добавить Prisma миграцией через `--create-only` после первого upsert. Связь с Prisma-таблицами — app-level через `metadata.externalId`. Подробности в `docs/guides/flowise-vs-nestjs.md` секция «F».
2. ✅ **Retriever-only без LLM — ЗАКРЫТ (2026-04-29).** Через `/api/v1/prediction/<id>` нельзя — Flowise валидирует ending node на категории `Chains/Agents/Engine` (`utils/index.js:283`). Issue [#3610](https://github.com/FlowiseAI/Flowise/issues/3610) `not planned`. **РЕШЕНИЕ:** использовать **Document Store API** — `POST /api/v1/document-store/vectorstore/query` возвращает `docs[{pageContent, metadata, id}]` без LLM-вызова. Тот же Postgres backend, тот же `catalog_chunks` table. Cost search: $0.0000004/query (только OpenAI embedding на query) вместо $0.0001 через Chatflow+Haiku. Подробности в `docs/guides/flowise-vs-nestjs.md` секция «E».
