...
| **1.B** Normalization | таблица `WaterAnalysis` с каноническими `params{hardness, iron, ...}`, enum `WaterSourceType`, PII-обезличиванием, derive из `WaterAnalysisRaw` | $0 (детерминированный transform, повторяемо без расходов) | ✅ закрыт 2026-05-06 |
...
| **4 backend** Map endpoints + USP | 7 endpoints для prostor-app карты + 4 USP-фичи: `GET /heatmap` (агрегаты PostGIS bbox+grid, 22 paramCode + risk), `GET /predict` (USP-1, kNN + interval-first 3 уровня + 4-level pdkStatus с median tipping + byCategory pre-grouping), `GET /depth-map` (USP-4 base, aquifer-buckets), `GET /depth-predict` (USP-4, interval-first depths + mostLikelyAquiferLayer), `GET /points` (high-zoom individual анализы, PII roundCoord 0.005°, **без orderNumber**), `POST /equipment-suggest` (USP-2 flagship cross-domain water→catalog, per-problem search + matchedProblem+reason), `GET /aquifer-stats` (USP-4 deep-dive, стратифицированная chemistry per layer). Throttle per-endpoint (10-120/min/IP), Redis cache TTL по типу (5мин predict / 10мин equipment-suggest / 1ч points / 24ч heatmap+depth-map+aquifer-stats). 1101 unit-тест в 64 test suites | $0 (read-only поверх derived dataset) | ✅ закрыт 2026-05-08 |
| **4.5** Frontend production | prostor-app карта на real backend: top-bar layer-icon + bottom-sheet с layer-toggles (heatmap по 5 параметрам / similar radius / depth-map + 3D extruded columns + aquifer chemistry / anomalies), пин клиента + radius circle, popup с CTA equipment-suggest, FTUX 3-step tour, dark theme map style, 3 viewport (mobile bottom-sheet / iPad drawer / desktop sidebar). Handoff promt v2 для claude design в `docs/management/water-map-design-prompt.md` (555 строк, 7 endpoints с sample responses, FSD-структура `views/water-map/`) | $0 | ⏳ в работе у claude design |
| **2.5** Geo-фильтр + ranking (similar) | `ST_DWithin` через Prisma raw поверх Flowise-managed `<storeId>_chunks.metadata->>'orderNumber'`; weighted re-rank по дистанции если потребуется. Заменено `/predict + /equipment-suggest` для большинства use-case'ов, но осталось как backlog для CRM-aqua dealer-side истории | $0 | ⏳ опционально |
| **3** Real-time + bridge с vision-catalog | webhook из CRM на новый анализ, авто-ingest pipeline. Equipment-matcher уже реализован в Phase 4 (`/equipment-suggest`) | infrastructural | ⏳ после Phase 4.5 |
