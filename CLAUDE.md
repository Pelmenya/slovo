...
3. ✅ **water-analysis** (Этапы 1.A + 1.A.5 + 1.B + Phase 2 закрыты 7 мая 2026, ~23 750 ₽ extraction + ~$0.30 embeddings = $217.83 Anthropic+OpenAI + Ahunter ≥ 2 000 ₽, 128 unit/e2e-тестов) — 15 504 бланка анализов воды Аквафор-Pro 2020-2026 нормализованы в structured dataset (21 paramCode + единицы + флаги, 97% normalize-clean, 93% holistic accuracy, 95.9% Mg coverage / 99.6-99.8% в свежих 2024-2026 после pdftotext-rescue), embedded через OpenAI text-embedding-3-large (3072 dim) в Flowise Document Store `water-analysis-aquaphor` (Custom Document Loader, 1 loader → 15 504 chunks за 176 секунд / 88 rec/s / $0.29). **Endpoint `POST /water-analysis/similar`** — vector search top-K через Flowise vectorstoreQuery (~600ms latency), детерминированный embedding-text builder тот же что при ingest, post-filter `matchIntakeType / regionContains / localityContains / depthRange` через oversample (×5 cap 250) + slice, `geo` зарезервирован (501 NotImplemented, ETA Phase 2.5 при появлении prostor-app). Throttle 60/min/IP, name-lookup storeId с single-flight cache (паттерн из catalog после фикса hardcoded UUID). Урок дня: **перед Vision OCR проверить text-based PDF / DOCX** (memory `feedback_check_pdf_text_first`). План: `docs/features/water-analysis.md`. Executive summary: `docs/management/water-analysis-executive-summary.md`. EDA: `docs/experiments/water-analysis/2026-05-06-stage-1b-eda.md`.
