...
5. ⏳ **catalog-ai-consultant** (2026-05-19, stub-план зафиксирован после PoC, реализация **не начата**, ждёт триггера): sticky AI-чат внизу страницы каталога PROSTOR — второй UX поверх той же RAG-инфры что smart-search. PoC `catalog-qa-poc-v1` (Sonnet 4.6 + ConversationalRetrievalQAChain + catalog-aquaphor DS) показал 7/7 reference Q&A с цитированием конкретных моделей за $0.04 / 3 ₽. Позиционирование: «первая линия 24/7 + квалификация лида», handoff к менеджеру при сложном запросе. Триггер запуска: Phase 1.5 backend closure + ≥30 карточек заполнено по Slice 6 ERP guide + ясность по multi-tenant chat persistence. См. `docs/features/catalog-ai-consultant.md` (stub) + `docs/experiments/knowledge-base-poc/2026-05-19-catalog-qa-baseline.md` (PoC закрыт).
6. ⏳ **notes-rag**: Q&A endpoint поверх knowledge-base — реактивация когда появится потребитель Phase 2 video/PDF-источников.
7. ⏳ **multi-tenant**: пользователи, JWT, биллинг (шаг к SaaS). Параллельно с domain-фичами, для каждой закладываем `userId` в модели с нуля (`KnowledgeSource` уже имеет `userId String? @db.Uuid`).
...
`.claude/state/HANDOFF.md` — короткая записка между сессиями. Автоматически читается через SessionStart hook (`load-context.js`) при старте новой сессии и добавляется в контекст.

**Правило обновления:** перед завершением крупной задачи или когда пользователь говорит, что заканчивает работу, обновляй `.claude/state/HANDOFF.md`. Содержание:
- **Текущая задача** — что сейчас в работе
- **Последние решения** — что приняли/decided за сессию
- **Следующие шаги** — что делать в начале next session
- **Open questions** — что не решено, нужно обсудить

**Формат жёсткий**: перезаписывай файл **целиком**, не накапливай историю. **Максимум 50 строк**. Точечно, не нарративно.

Backups транскриптов сохраняются автоматически в `.claude/backups/pre-compact-*.jsonl` через PreCompact hook (gitignored).
...
- **Evidence-first (против conclusion-anchoring)** — любое ✅/❌-утверждение о поведении внешнего тула или системы — только с воспроизводимым тестом или ссылкой на source/строку. Без пруфа → писать `UNVERIFIED-assumption`, не факт. Порядок жёсткий: сначала тест → потом запись. Не подгонять факты под уже зафиксированный вывод. Прецедент: A3 OpenCode MCP permissions прошли 4 версии за одну сессию из-за conclusion-anchoring (зафиксировано в `experiments/agentic-core/opencode-spike-2026-06-22.md` § Errata).
